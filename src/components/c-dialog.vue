<template>
    ...
    >
        <div
            ...
            :class="[c_class, c_options.dialogClass]"
        ...
        >
            <div
                class="dialog_mask"
                ref="mask"
                :style="{ background: c_options.mask.color }"
            ></div>
            <div
                class="dialog"
            ...
            >
                <div class="dialog_container">
                    ...
                    <div
                        v-if="c_options.dialog_body"
                        class="dialog_body"
                        :style="c_options.dialog_body_style"
                    >
                        <slot name="body">
                            {{ c_options.dialog_body }}
                        </slot>
                    </div>
                    <div v-if="c_options.dialog_footer" class="dialog_footer">
                        <slot name="footer"></slot>
                        <div
                            v-if="c_options.dialog_btn_footer"
                            class="dialog_footer_btns"
                        >
                            <div
                                class="dialog_btn dialog_btn-confirm"
                                v-if="!c_options.hideConfirmBtn"
                                @click="close(true)"
                            >
                                {{ c_options.confirmBtnText || "Confirm" }}
                            </div>
                            <div
                                class="dialog_btn dialog_btn-cancel"
                                v-if="!c_options.hideCancelBtn"
                                @click="close(false)"
                            >
                                {{ c_options.cancelBtnText || "Cancel" }}
                            </div>
                        </div>
                    </div>
                </div>
...
<script lang="ts">
import Vue from "vue";
import { judge_is_dom, get_el_pos, adjustment_pos } from "../unit/unit";
    ...
    methods: {
        ...
        pop_in_before_enter(el) {
            ...
            let pop_animation = anime({
                ...
                update: ani => {
                    let target = ani.animatables[0].target;
                    if (this.c_is_show) {
                        this.correcting_pos(this.c_options.dialog_pos);
                    }
                    $(dialog_el).css({ opacity: target.opacity });
                    $(dialog_el).css({ transform: `scale(${target.scale})` });
                },
                complete: elements => {
            ...
            $(el).data("pop_animation", pop_animation);
        },
        pop_in_enter(el, done) {
            let dialog_el = this.$refs.dialog;
            let pop_animation = $(el).data("pop_animation");
            pop_animation.complete = () => {
                this.$emit("dialog_enter");
                done();
            };
            setTimeout(() => {
